...
float MAX_JOINT_VEL = 0.05;  //in radians/sec
int SOUND= 0;
...
public:
  IKTrajectoryExecutor(){
    ...
    service = node.advertiseService("execute_cartesian_ik_trajectory_l", 
...
ik_request.ik_request.ik_seed_state.joint_state.name.push_back("l_shoulder_pan_joint");
    ...
    if(SOUND == 0){
      ...
      system("rosrun sound_play say.py \"I am performing your sequence\"");
      SOUND = 1;
    }
  ...
  bool execute_cartesian_ik_trajectory(
         pr2lib::ExecuteCartesianIKTrajectory::Request &req,
         pr2lib::ExecuteCartesianIKTrajectory::Response &res){

    int trajectory_length = req.poses.size();
    int i, j;
    MAX_JOINT_VEL = req.header.seq/100.0;
    ROS_INFO("Using Max Joint Velocity: %0.3f", MAX_JOINT_VEL);
...
int main(int argc, char** argv){
  ...
  //init ROS node
  ros::init(argc, argv, "cartesian_ik_trajectory_executor_l");
